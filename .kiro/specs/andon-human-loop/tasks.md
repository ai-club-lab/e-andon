...
- [x] 10.1 (P) 集計 API（`/analytics/pareto`・`/analytics/accuracy`・`/analytics/recurrence`。純関数 analytics.py に分離・open は既定停止時間 300s でクリップ・データなしは empty 明示。※カテゴリは rca 由来 — 訂正はカテゴリを持たないため「訂正後カテゴリ優先」は現状未適用）— 7.2, 7.3, 7.5, 7.7
- [x] 10.2 再発検知（窓×同一カテゴリ≥3 で alert。suggestion は定型文＋当該カテゴリの直近の人手訂正原因を引用。閾値判定は決定論）— 7.4
- [x] 10.3 `static/analytics.html`（パレート棒+累積折れ線・正答率日次折れ線をインライン SVG 依存ゼロ・期間切替 7/30日）— 7.2, 7.3, 7.5
- [x] 10.4 サイドメニュー「準備中」2項目→「📊 分析・履歴」導線に置換（設定のみ残置）＋棒クリック→イベント一覧ドリルダウン（クライアント側フィルタ・裁定状態・/e/{id} リンク）— 7.1, 7.6
- [x] 10.5 集計関数のフィクスチャテスト（パレート順序・累積比・open クリップ・空期間・再発閾値・日次正答率）— 7.2, 7.4, 7.7 ✅TDD 5テスト、計51緑。実ブラウザで描画・ドリルダウン・実RCA自己分類（conveyance）を確認
