...
- [ ] 3.1 PoC（`docs/poc/detect_v2.py`）を `services/detector` の再利用モジュールへ昇格（`detect_frame -> FrameResult`）— 2.1, 2.2, 2.3, 2.4
- [ ] 3.2 判定根拠の構造化記録（FlagDetail に数値・シグナル種別）— 2.7, 10.4
- [ ] 3.3 時系列トラッキングで同一変位部品を1 `AnomalyEvent` に集約（新規確定で一度だけ発火、閾値未満継続で close）— 3.1, 3.2, 3.3, 3.4
- [ ] 3.4 代表フレームを Cloud Storage(private) に保存し `rep_frame_uri` を格納 — 3.2, 5.2
- [ ] 3.5 境界帯のみ Gemini Vision で確認（`confirm_with_gemini`、anyOf/default サニタイズ・429リトライON・us-central1）— 2.5, (研究#4)
- [ ] 3.6 (P) 検出部品<4 のフレームを対象外化＋記録 — 2.6
- [ ] 3.7* トラッキングと閾値判定のユニットテスト（正常フレームでfalse positiveゼロ／異常イベント1件）— 2, 3

## 4. 擬似ストリーム配信

- [ ] 4.1 映像を5fpsサンプリング＋末尾ループ再生する配信ループ — 1.1, 1.2
- [ ] 4.2 検知オーバレイ（ベースライン・部品マーカー・異常マーカー）を重畳 — 1.3
- [ ] 4.3 SSE エンドポイント `GET /stream` でフレーム逐次配信 — 1.4
- [ ] 4.4 映像ソース読み込み失敗時のエラー記録＋停止状態表示 — 1.5
...
- [ ] 7.1 映像(SSE)＋IoT時系列を同一画面に表示するUI — 1.4, 7.1
- [ ] 7.2 異常イベントの視覚強調＋直近一覧（`GET /events`）— 7.2, 7.3
- [ ] 7.3 IoT照会 API `GET /iot?channel&t0&t1` とUI連携 — 6.2, 7
- [ ] 7.4 チャット `POST /chat`：異常通知の受信表示＋対話照会（agent tools 経由）— 6.1, 6.2, 6.3
- [ ] 7.5 範囲外照会時の「データ無し」明示 — 6.4
