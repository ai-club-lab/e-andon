---
title: "チョコ停を検知して\"なぜ\"まで答える：決定論CV × ADKエージェントでHITL閉ループを作った話【2026年版】"
emoji: "🏭"
type: "tech"
...
このハッカソン（DevOps × AI Agent、2026年）では、そこを**1本の閉ループ**にしました。映像から整列異常を**決定論CV**（コンピュータビジョン）で検知し、**AIエージェント**が振動ログを相関して原因を推定、チャットで通知し、人が正誤を確認・訂正して蓄積、次回の推論に還す（**HITL: Human-in-the-Loop**）。GCP 上で稼働URLまで到達しています。
...
## 3. 原因推定：ADKエージェントを「オーケストレータ＋ツール」で組む

異常が確定したら、**なぜ**の推論（RCA: Root Cause Analysis、原因分析）に入ります。ここでようやくエージェントの出番です。基盤は Google の **Agent Development Kit (ADK)** を使い、モデルは Vertex AI の Gemini 2.5 Flash（**ADC: Application Default Credentials による認証なのでAPIキー不要**）。
...
agent = Agent(
    ...
    instruction=_INSTRUCTION,      # 「振動を相関確認し、過去事例を検索して原因を推定」
    tools=[query_vibration, query_logs, search_past_cases, get_frame],
)
```

IoT は実機がないので、**映像タイムラインに整合した合成振動**を生成し、異常が通過する瞬間に **X軸加速度スパイク＋高調波** を注入しています。こうするとエージェントの推論に「効く」物理的な根拠が生まれます。

実際に稼働URLで動かすと、エージェントは `query_vibration` を呼んで異常時刻の振動を確認し、こう返してきました。

> 推定原因: **振動（X軸）の異常**（確信度 70%）
> 根拠: vibration_x の max_abs が 3.559 で最も大きく逸脱; motor_current 3.45; temperature 42℃は正常範囲

指示に「逸脱は絶対値でなく通常レンジからの相対で評価せよ」と一文足すだけで、温度42℃（実は正常）に釣られず、注入したX軸スパイクを主因に選ぶようになりました。プロンプトの効きどころが分かる良い例でした。
